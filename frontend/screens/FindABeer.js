    ...
    //Function to make beer container clickable
    const BeerItem = ({ item }) => {
        const handleClick = () => {
          //Handle the click event
          console.log('Beer item clicked:', item.beerName);
        };
      
        return (
          <TouchableOpacity style={styles.containerItem} onPress={handleClick}>
            <View style={styles.leftContainer}>
              <Text style={styles.beerName}>{item.beerName}</Text>
              <Text style={styles.beerName}>Price: ${item.beerPrice}</Text>
            </View>
            <View style={styles.rightContainer}>
              {[1, 2, 3, 4, 5].map((star) => (
                <Ionicons
                  ...
                  name="star"
                  size={20}
                  color={star <= item.rating ? COLORS.foam : COLORS.grey}
                />
              ))}
            </View>
          </TouchableOpacity>
    ...
    const FindABeer = ({ navigation }) => {
        ...
        const beerData = [
            { beerName: 'Beer Name 1', beerPrice: 9, rating: 3 },
        ];
